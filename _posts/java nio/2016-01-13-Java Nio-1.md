---
layout: post
title: Java Nio系列教程-Buffer
categories: [Java,Nio]
description: Java Nio系列教程|Buffer
keywords: Java,Nio,Buffer
autotoc: true
comments: true
---

本文转自并发编程网<http://ifeve.com/buffers/>

## 一、Channel

```java
//读取文件到内存
RandomAccessFile rf = new RandomAccessFile("d://a.txt");
//把文件读到通道
FileChannel fic = rf.getChannel();
//创建字节缓冲区
ByteBuffer buffer = ByteBuffer.allocate(48);
//读取通道数据到Buffer
int byteRead = fic.read(buf);
while(byteRead != -1){
    
    Syetem.out.println("Read "+byteRead);
	buffer.flip();//make buffer ready for read
	while(buffer.hasRemaining()){
	   System.out.println((char)buffer.get()); // read 1 byte at a time
	}
	buffer.clear();//make buffer ready for writing
	byteRead = fic.read(buf);
}
rf.close();
```
...
![](http://7xptxy.com1.z0.glb.clouddn.com/Java%20Nio-1.png)
